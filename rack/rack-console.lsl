  ...
  link_message(integer from, integer chan, string msg, key xyzzy) {
	...
	if (workout == "[RESET]" || workout == "[time out]") {
	  ...
	  llSetLinkPrimitiveParamsFast(display_link,
					...
					PRIM_NORMAL, 0, "setup-norm", <1,1,0>,ZERO_VECTOR,0,
					PRIM_SPECULAR, 0, "setup-spec", <1,1,0>,ZERO_VECTOR,0, <1,0.5,0>, 60, 15]);
	  llMessageLinked(LINK_THIS, 0,"", "fw_data: Weight");
	  return;
	}
	PUSH(workout);
	llSetLinkPrimitiveParamsFast(display_link,
				     [PRIM_TEXTURE, 0, "-"+workout, <1,1,0>,ZERO_VECTOR,0,
				      PRIM_NORMAL, 0, "-" + workout + "-norm", <1,1,0>,ZERO_VECTOR,0,
				      PRIM_SPECULAR, 0, "-" + workout + "-spec", <1,1,0>,ZERO_VECTOR,0, <1,0.5,0>, 60, 15]);
	debug((string)next);
	NEXT_STATE;
	break;
      }
