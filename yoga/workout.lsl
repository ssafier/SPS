...
//----------------------------------
string export2Json(key yogi, string record) {
...
default {
  state_entry() {
    ...
    case EndSession: {
      string record = llGetSubString(msg,1,-1);
      debug(export2Json(xyzzy, record));
      llMessageLinked(LINK_THIS, saveLifterStats,
		      "|yogaSession|" + export2Json(xyzzy, record),
		      xyzzy);
      break;
    }
    case returnLeaf: {
	...
	break;
      ...
      case "[JoinClass]": {
	llMessageLinked(LINK_THIS, joinClass, "", yogi);
	break;
      }
      case "[LeaveClass]": {
	llMessageLinked(LINK_THIS, leaveClass, "", yogi);
	break;
      }
      case "[time out]": break;
      default: {
  ...
  changed(integer f) {
	...
	sitting) { /* &&  llAvatarOnSitTarget() == NULL_KEY) { */  // BUG -- does not return false on log out
      ...
      if (time > 60) {
	if (time > 1800) time = 1800;
	llMessageLinked(LINK_THIS, SaveLog, (string) EndSession + "|5|0|"+ (string)((FAT_CHANGE) * time) + "|" + (string) ALL_BODY_PARTS, yogi);
      }
      SayToHud("end|");
